...
def get_server_stats():
    try:
        ...
        total_scan_count = NucleiScanner().active_scans_count()

        nuclei_status = NucleiScanner().check_nuclei_is_installed()

        if nuclei_status:
            nuclei_status = "online"
        else:
            nuclei_status = "offline"
        ...
        if nuclei_status == "online":
            logging.info("Getting targets")
            target_response = get_targets(total_scan_count, 3)
            ...
            if target_response['success']:
                ...
                if targets is not None:
                    for target in targets:
                        ...
                        start_scan_response = NucleiScanner().start_scan(target)
...
def send_scan_telemetry():
    try:
        print("Sending scan telemetry")
        scan_results = NucleiScanner().get_all_results()
        ...
        for result in scan_results:
            ...
            result['agent_type'] = "nuclei"

        json_result = json.dumps(scan_results, indent=4)
        print("Sending scan results: ", json_result)

        response = requests.post('https://panel.hunterbounter.com/scan_results/nuclei/save', data=json_result,
